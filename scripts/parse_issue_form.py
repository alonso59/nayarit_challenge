...
def extract_first_http_url(value: str, field: str) -> str:
    match = HTTP_URL_RE.search(value)
    if not match:
        raise ValueError(
            f"{field} must contain the uploaded attachment link generated by GitHub."
        )
    return require_http_url(match.group(0).rstrip(".,;"), field)
...
def build_metadata(event: dict) -> dict:
    ...
    metadata = {
        ...
        .isoformat()
        .replace("+00:00", "Z"),
        "submission_zip_url": extract_first_http_url(
            values["submission_zip"].strip(), "submission_zip"
        ),
